...
def apply(
    ...
    """Applies a slide-out transition to the subtitle text and optional stroke outline.
    ...
    # Apply slide-out to text_clip pos_fn
    def fill_pos_fn(t):
        if t >= (cue_duration - slide_duration):
            progress = (t - (cue_duration - slide_duration)) / slide_duration
            ease_progress = progress**2

            if direction == "left":
                x = target_x - (target_x + fill_w) * ease_progress
                return (int(x), target_y)
            elif direction == "right":
                x = target_x + (video_w - target_x) * ease_progress
                return (int(x), target_y)
            elif direction == "top":
                y = target_y - (target_y + fill_h) * ease_progress
                return (target_x, int(y))
            elif direction == "bottom":
                y = target_y + (video_h - target_y) * ease_progress
                return (target_x, int(y))

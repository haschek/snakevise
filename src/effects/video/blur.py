...
def apply(clip: VideoClip, strength: float) -> VideoClip:
    """Applies a progressive blur effect starting from the midpoint of the clip.

    Args:
        clip: The video clip to process.
        strength: The intensity of the blur (typically 1 to 10).

    Returns:
        The processed clip with progressive blur.
    """
    w, h = clip.size
    short_side = min(w, h)

    # Calculate max sigma (e) based on strength and shorter side.
    # strength 10 -> 5% of short side (strong blur)
    # strength 1 -> 0.5% of short side (subtle blur)
    ...
    def filter_frame(get_frame, t):
        ...
        # Linear interpolation from midpoint to end
        factor = (t - midpoint) / midpoint
